...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (feature.Properties.Any(p => p.Key == PropData.Highway && MapFeature.HighwayTypes.Any(v => p.Value.StartsWith(v))))
        {
        ...
        else if (feature.Properties.Any(p => p.Key == PropData.Water) && feature.Type != GeometryType.Point)
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == PropData.Railway))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == PropData.Natural && featureType == GeometryType.Polygon))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == PropData.Boundary && p.Value.StartsWith("forest")))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == PropData.Landuse && (p.Value.StartsWith("forest") || p.Value.StartsWith("orchard"))))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
            baseShape = geoFeature;
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == PropData.Landuse && (p.Value.StartsWith("residential") || p.Value.StartsWith("cemetery") || p.Value.StartsWith("industrial") || p.Value.StartsWith("commercial") ||
                                                        ...
                                                        p.Value.StartsWith("brownfield"))))
        {
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == PropData.Landuse && (p.Value.StartsWith("farm") || p.Value.StartsWith("meadow") || p.Value.StartsWith("grass") || p.Value.StartsWith("greenfield") ||
                                                        p.Value.StartsWith("recreation_ground") || p.Value.StartsWith("winter_sports") || p.Value.StartsWith("allotments"))))
        {
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                 ...
                 feature.Properties.Any(p => p.Key == PropData.Landuse && (p.Value.StartsWith("reservoir") || p.Value.StartsWith("basin"))))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == PropData.Building))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == PropData.Leisure))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == PropData.Amenity))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
